...
// Объявляем Group для произвольного типа String
// синонимом vector.
...
vector<Group<String>> GroupHeavyStrings(vector<String> strings) {
  ...
  for(auto& str : strings){
    set<typename String::value_type> words(str.begin(), str.end());
    groups[move(words)].push_back(move(str));
  }
  for(auto& [key, item] : groups){
    res.push_back(move(item));
  }
  return res;
}
